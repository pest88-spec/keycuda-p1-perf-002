...
// P1-PERF-001: CUDA Stream Manager for parallel execution
#include "utils/cuda_stream_manager.h"
...
StepResult GpuExecutor::Execute() {
    ...
    if (verbose_) {
        std::cout << "[debug] Launching separated kernels with CUDA streams (P1-PERF-001) grid=" << config_.grid.x
                  << " block=" << config_.block.x
                  << " points/thread=" << config_.points_per_thread << std::endl;
    }
    ...
    // 策略: stream0→ECC, stream1→Hash; 使用cudaEvent_t同步批次依赖
    // 性能目标: 1.2-1.5× 吞吐量提升 (通过流水线并行)
    try {
        ...
        if (event_err != cudaSuccess) {
            throw std::runtime_error(std::string("cudaEventCreate failed: ") + cudaGetErrorString(event_err));
        }
        ...
        try {
            ...
            if (event_err != cudaSuccess) {
                throw std::runtime_error(std::string("cudaEventRecord failed: ") + cudaGetErrorString(event_err));
            }

            // 流1: Hash计算和地址比对 (40个寄存器/线程)
            // Hash依赖ECC结果,使用cudaStreamWaitEvent确保依赖关系
            event_err = cudaStreamWaitEvent(stream_hash, ecc_complete, 0);
            if (event_err != cudaSuccess) {
                throw std::runtime_error(std::string("cudaStreamWaitEvent failed: ") + cudaGetErrorString(event_err));
            }
            ...
            if (verbose_) {
                std::cout << "[debug] Dual-stream kernels completed (ECC stream0 + Hash stream1)" << std::endl;
            }
        } catch (...) {
        ...
        throw std::runtime_error(std::string("CUDA dual-stream execution failed: ") + e.what());
    }
